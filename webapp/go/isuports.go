...
func dispenseID(ctx context.Context) (string, error) {
	...
	return strconv.Itoa(rnd.Int()), nil
}
...
func batchRetrieveCompetitions(ctx context.Context, tenantDB dbOrTx, ids []string) ([]*CompetitionRow, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	...
	if err := tenantDB.SelectContext(ctx, &competitions, query, params...); err != nil {
...
func playersAddHandler(c echo.Context) error {
	...
	now := time.Now().Unix()
	newPlayers := make([]PlayerRow, 0, len(displayNames))
	for _, displayName := range displayNames {
		...
		newPlayers = append(newPlayers, PlayerRow{
			TenantID:       v.tenantID,
			ID:             id,
			DisplayName:    displayName,
...
func playerHandler(c echo.Context) error {
	...
	rawQuery := "SELECT * FROM player_score WHERE player_id = ? AND competition_id in (?) ORDER BY competition_id, row_num DESC"
	...
	for _, ps := range pss {
		comp, ok := compMap[ps.CompetitionID]
		if !ok {
			return fmt.Errorf("error competition not found: %s", ps.CompetitionID)
		}
